...
public class RoomActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        editor.putInt("domain_id",domain_id);
        editor.apply();
        ...
        // 传入回调接口
        ThreadManager.setCallbackToInRoomSub(new DataCallback<InRoom>() {
            @Override
            public void onDataReceived(InRoom result) {
                runOnUiThread(() -> {
                    // 以下内容在主线程执行
                    if (result.room_id != null && result.room_id.equals(roomId)) {
                        if(result.room_state.equals("waiting")){
                            updatePlayers(result);
                        }else if(result.room_state.equals("playing")){
                            startGame();
                        }else if(result.room_state.equals("empty")){
                            finish();
                        }
                    }
                });
        ...
        ThreadManager.setCallbackToPlayerColorMappingsSub(new DataCallback<PlayerColorMappings>() {
            @Override
    ...
    public void startGame(){
        ...
        intent.putExtra("nickname", currentPlayerNickname);
        intent.putExtra("color",getCurrentPlayerColor());
    ...
    private void updateUI() {
        ...
        Toast.makeText(this,"有新玩家进入游戏",Toast.LENGTH_SHORT).show();
        // 只有房主显示开始游戏按钮

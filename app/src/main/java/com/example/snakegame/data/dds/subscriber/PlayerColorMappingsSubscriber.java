...
public class PlayerColorMappingsSubscriber {
    private static final String TAG = "PlayerColorMappingsSubscriber";
    private static int DOMAIN_ID = 6;
    private static DomainParticipant dp;
    private static Subscriber sub;
    private static Topic tp;
    private static PlayerColorMappingsDataReaderListener listener;
    private static PlayerColorMappingsDataReader reader;
    public static String roomId = "";
    public static PlayerColorMappings rightData;
    ...
    public static void initialize(){
        ...
        reader = (PlayerColorMappingsDataReader)dr;
        Log.d(TAG, "create dr succeeded");
    }

    public static void receiveData(String room_id, DataCallbackColorMap callback) {
        roomId = room_id;
        ...
        while (true) {
            ...
            if(success){
                callback.savePlayerColorMapping(rightData);
                break;
            }
        }
    }
...
class PlayerColorMappingsDataReaderListener implements DataReaderListener {
    public void on_data_available(DataReader dataReader) {
        ...
        for (int i = 0; i < infoSeq.length(); ++i) {
            ...
            if (receivedData.room_id != null && receivedData.room_id.equals(PlayerColorMappingsSubscriber.roomId)) {
                PlayerColorMappingsSubscriber.rightData = receivedData;
                PlayerColorMappingsSubscriber.success = true;
            }
        }
